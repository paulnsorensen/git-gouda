...
The workflow refuses to publish if the tag isn't an ancestor of the discovered default branch:

```bash
default_branch="$(gh repo view "$GITHUB_REPOSITORY" --json defaultBranchRef --jq '.defaultBranchRef.name')"
git fetch origin "$default_branch"
if ! git merge-base --is-ancestor "$GITHUB_SHA" "origin/$default_branch"; then
  echo "::error::Tag … is not on origin/$default_branch. Refusing."
  exit 1
fi
```

This pairs with the default-branch ruleset: if direct pushes to the default branch are blocked, then everything reachable from it has been through PR review, and a tag on that branch therefore covers only reviewed code. A tag pushed straight to a side branch would bypass that — the guard catches it.
...
The workflow runs, verifies the tag is on the discovered default branch, calls `gh release create v1.0.0 --generate-notes`, and the release shows up in the Releases tab with categorized notes.
