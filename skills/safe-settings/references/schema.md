...
Reference for the keys consumed by `github-community-projects/safe-settings`. This skill only ships a minimal baseline in `assets/settings.yml`; everything below is what's available when the user wants to extend it.

Authoritative source: <https://github.com/github-community-projects/safe-settings/blob/2.1.17/docs/sample-settings/settings.yml>
...
A key set at repo level overlays the same key at suborg level, which overlays org level. Arrays merge by identity for supported objects such as labels, names, logins, types, and contexts; unmatched entries remain. Org-level rulesets are applied at the organization layer; lower-scope files must not be treated as weakening an active org rule. Verify behavior against the pinned 2.1.17 merge implementation before relying on it.
...
conditions:
  ref_name:
    ...
    exclude: []
  # Required for org-level rulesets. Drop this selector at suborg/repo level;
  # those files use the repository endpoint and should not copy this shape.
  repository_name:
    include: ["~ALL"]
    exclude: ["legacy-*"]
...
An active organization ruleset remains an organization constraint. A suborg or repository file can add compatible settings, but it cannot weaken that active rule.
