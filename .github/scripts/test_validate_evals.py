...
class ValidateEvalsMainTest(unittest.TestCase):
    ...
    def _write_skill(self, skill: str) -> None:
        path = self.tmpdir / "skills" / skill / "SKILL.md"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(f"---\nname: {skill}\ndescription: test\n---\n", encoding="utf-8")
    ...
    def test_missing_eval_for_skill_fails(self) -> None:
        self._write_skill("foo")
        rc, _, err = self._run()
        self.assertEqual(rc, 1)
        self.assertIn("missing required evals/evals.json", err)

    def test_mismatched_skill_name_fails(self) -> None:
        self._write_skill("foo")
        self._write_eval("foo", {**VALID_EVALS, "skill_name": "bar"})
        rc, _, err = self._run()
        self.assertEqual(rc, 1)
        self.assertIn("does not match skill directory", err)
    ...
    def test_shipped_eval_files_validate(self) -> None:
        ...
        for ef in eval_files:
            with self.subTest(file=str(ef.relative_to(REPO_ROOT))):
                self.assertEqual(validate_evals.validate_file(ef, ef.parent.parent.name), [])
