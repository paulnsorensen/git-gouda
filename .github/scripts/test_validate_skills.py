...
class ResourceAndCatalogTest(unittest.TestCase):
    def test_missing_linked_resource_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            self.assertTrue(validate_skills.validate_links(path))

    def test_missing_pointer_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "skills" / "foo" / "SKILL.md"
            path.parent.mkdir(parents=True)
            path.write_text("---\nname: foo\ndescription: test\n---\n\nSee `references/nope.md`.\n", encoding="utf-8")
            self.assertTrue(validate_skills.validate_resource_pointers(path))

    def test_link_escape_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            skill = root / "skills" / "foo"
            skill.mkdir(parents=True)
            ...
            errors = validate_skills.validate_links(path)
            self.assertTrue(errors)
            self.assertIn("escapes skill root", errors[0])

    def test_existing_external_pointer_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            skill = root / "skills" / "foo"
            skill.mkdir(parents=True)
            ...
            path = skill / "SKILL.md"
            path.write_text(
                "---\nname: foo\ndescription: test\n---\n\nSee " + chr(96) + "assets/../../outside.txt" + chr(96) + ".\n",
                encoding="utf-8",
            )
            errors = validate_skills.validate_resource_pointers(path)
            self.assertTrue(errors)
            self.assertIn("escapes skill root", errors[0])
            self.assertEqual(outside.read_text(encoding="utf-8"), "external")

    def test_nested_page_resources_are_checked(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            skill = root / "skills" / "foo"
            ...
            path = refs / "guide.md"
            path.write_text("[missing](assets/nope.md)\n", encoding="utf-8")
            self.assertTrue(validate_skills.validate_bundle(skill / "SKILL.md"))
